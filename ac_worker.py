...
class ACWorker(object):
    ...
    """
    def __init__(self, worker_num, shared_network, device, filewriter, saver,  worker_type="play"):
    ...
    def replay(self, sess, iteration, rb):
        """
        Threading Operation for a replay worker
        """

        # Don't start replaying until we've learned some knowledge
        if rb.size() < ACWorkerConfig.REPLAY_START:
            return

        print rb.size()
        # First sync parameters
        self.network.copy_params_from_shared_network(sess)

        # Pull experience from memory
        batch_s, batch_a, batch_r, batch_td = rb.sample_batch(self.batch_size)

        # Calculate the gradientsm loss and the global norm
        feed_dict = {self.network.s: np.stack(batch_s), self.network.a: batch_a, \
            self.network.td: batch_td, self.network.r: batch_r, self.network.ep_score:0.0}
    ...
    def run(self, sess, iteration, rb):
        """
        ...
        for s, a, r, sp, t, v in transitions:
            ...
            # Add to replay buffer for replay threads
            rb.add((s, one_hot_a, R, td))
